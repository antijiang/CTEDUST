...
void SYS_Init(void)
{
  ...
  /* IP clock source */
  SYSCLK->CLKSEL1 = SYSCLK_CLKSEL1_UART_IRC22M | SYSCLK_CLKSEL1_PWM23_IRC22M |
      ...
      if (Update_ReadCount >= UPDATE_CYCLIC_COUNT)
        {
          ...
          if (CurrentUARTMod == UARTMOD_CAL)Current_Machine_State=0; //2015.0606
          else
            {
              // 提高无尘电压
              // 获取机器工作状态
