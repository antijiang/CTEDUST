...
volatile uint16_t VB1Temp;//温度校准后对应的电压
...
//startpos : 数据开始点 （往回）
uint16_t
Cyclic_Aver(uint16_t* buf, uint8_t BUFSIZE, uint8_t startpos,
...
//返回 温度校准后的电压
uint16_t
CalTempCalibrate(uint16_t orgv)
{
  return orgv;
}
void
...
SYS_Init(void)
{
  ...
  SYSCLK ->APBCLK = SYSCLK_APBCLK_UART0_EN_Msk| /*SYSCLK_APBCLK_SPI0_EN_Msk |*/
		  SYSCLK_APBCLK_PWM23_EN_Msk
                    | SYSCLK_APBCLK_PWM45_EN_Msk | SYSCLK_APBCLK_ADC_EN_Msk
  ...
  /* IP clock2 source */
  SYSCLK ->CLKSEL2 = SYSCLK_CLKSEL2_PWM45_IRC22M;
...
//功能：从当前位置（数据采样当前位置，数据最新）往回计算最新的 AVGDataNum个数据的平均值
...
ProcessInterfaceCommand(void)
{
  ...
  if (CommandReceived)
    {
      memcpy((char *) &Buf, (char *) &ReadBuffer, SERIAL_BUFF_SIZE);
